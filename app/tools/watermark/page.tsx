...
export default function WatermarkPage() {
  ...
  const handleWatermark = async () => {
    ...
    try {
      ...
      // 2. Add watermark
      const processRes = await fetch("/api/tools/watermark", {
        method: "POST",
      ...
      if (!processRes.ok) {
        throw new Error(processData.error || "Watermark process failed")
      }
      ...
      <div className="bg-surface-container p-8 rounded-xl border border-outline-variant max-w-4xl mx-auto">
        {result ? (
          <div className="text-center py-8">
            ...
            <h1 className="font-display-lg text-display-lg mb-2">Watermark Added Successfully!</h1>
          ...
          <>
            <div className="text-center mb-8">
              ...
              <h1 className="font-display-lg text-display-lg">Add Watermark</h1>
              <p className="text-on-surface-variant font-body-md mt-2">Stamp text or a watermark across all pages of your PDF.</p>
            </div>
            ...
            {!file ? (
              ...
              <div className="mt-8">
                ...
                <div className="bg-surface border border-outline-variant rounded-xl p-6 mb-8 space-y-6">
                  <div>
                    <label className="block text-label-md font-bold uppercase mb-2 text-on-surface-variant">Watermark Text</label>
                    <input 
                      type="text" 
                      value={watermarkText}
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      <label className="block text-label-md font-bold uppercase text-on-surface-variant">Opacity</label>
                      <span className="font-mono-sm text-primary font-bold">{Math.round(opacity * 100)}%</span>
                    </div>
                    <input 
                      type="range" 
                      min="0.1" 
                ...
                <button 
                  ...
                  disabled={uploading || processing || !watermarkText.trim()}
                ...
                >
                  {uploading ? "Uploading..." : processing ? "Processing..." : `Add Watermark`}
                </button>
              </div>
            )}
          </>
