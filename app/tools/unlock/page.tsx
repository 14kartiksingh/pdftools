...
export default function UnlockPdfPage() {
  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      // 2. Dispatch job
      const processRes = await fetch("/api/tools/unlock", {
        method: "POST",
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (data.status === 'COMPLETED') {
          ...
          clearInterval(interval);
          setError(data.error || "Failed to unlock PDF. Is the password correct?");
          setProcessing(false);
          setJobId(null);
        }
      } catch (err: any) {
      ...
      <div className="bg-surface-container p-8 rounded-xl border border-outline-variant max-w-3xl mx-auto">
        {result ? (
          <div className="text-center py-8">
            ...
            <h1 className="font-display-lg text-display-lg mb-2">PDF Unlocked Successfully!</h1>
            <p className="text-on-surface-variant font-body-md mb-8">Password protection has been permanently removed.</p>
          ...
          <>
            <div className="text-center mb-8">
              ...
              <h1 className="font-display-lg text-display-lg">Unlock PDF</h1>
              <p className="text-on-surface-variant font-body-md mt-2">Remove password security from your encrypted PDF documents.</p>
            </div>
            ...
            {!file ? (
              <div className="border-2 border-dashed border-outline-variant rounded-xl p-12 text-center hover:border-primary transition-colors cursor-pointer relative bg-surface">
                ...
                <p className="font-body-lg text-body-lg text-on-surface mb-2">Drag & drop a protected PDF</p>
              ...
              <div className="mt-8">
                ...
                <div className="bg-surface border border-outline-variant rounded-xl p-6 mb-8 space-y-6">
                  <div>
                    <label className="block text-label-md font-bold uppercase mb-2 text-on-surface-variant">Enter PDF Password</label>
                    ...
                    <p className="text-label-sm font-label-sm text-on-surface-variant mt-2">You must know the current password to unlock the document.</p>
                  </div>
                </div>

                <button 
                  onClick={handleUnlock}
                  disabled={uploading || processing || !password}
                ...
                >
                  ...
                  <span className="relative z-10">
                    {uploading ? "Uploading..." : processing ? `Unlocking (${progress}%)...` : "Unlock PDF"}
                  </span>
                </button>
              </div>
            )}
