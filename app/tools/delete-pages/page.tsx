...
export default function DeletePagesPage() {
  ...
  const handleDeletePages = async () => {
    ...
    try {
      ...
      // 2. Delete pages
      const deleteRes = await fetch("/api/tools/delete-pages", {
        method: "POST",
      ...
      if (!deleteRes.ok) {
        throw new Error(deleteData.error || "Delete operation failed")
      }

      setResult(deleteData.file)
      router.refresh()
    } catch (err: any) {
      ...
      <div className="bg-surface-container p-8 rounded-xl border border-outline-variant max-w-4xl mx-auto">
        {result ? (
          <div className="text-center py-8">
            <div className="w-16 h-16 mx-auto bg-error-container text-on-error-container rounded-full flex items-center justify-center mb-6">
              <span className="material-symbols-outlined text-4xl">check_circle</span>
            </div>
            <h1 className="font-display-lg text-display-lg mb-2">Pages Deleted Successfully!</h1>
          ...
          <>
            <div className="text-center mb-8">
              ...
              <h1 className="font-display-lg text-display-lg">Delete Pages</h1>
              <p className="text-on-surface-variant font-body-md mt-2">Select specific pages to remove from your PDF.</p>
            </div>
            ...
            {!file ? (
              ...
              <div className="mt-8">
                ...
                <button 
                  ...
                  disabled={uploading || deleting || selectedPages.length === 0}
                  className="w-full bg-error text-on-error p-4 rounded font-bold uppercase tracking-widest hover:brightness-110 transition-all disabled:opacity-50 mt-8"
                >
                  {uploading ? "Uploading..." : deleting ? "Deleting..." : `Delete ${selectedPages.length} Pages`}
                </button>
              </div>
            )}
          </>
