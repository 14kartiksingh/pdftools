...
export async function POST(req: Request) {
  try {
    ...
    if (!fileId || !pages || !Array.isArray(pages) || pages.length === 0) {
      return NextResponse.json({ error: "File ID and ordered pages are required" }, { status: 400 })
    }
    ...
    // 3. Create new PDF and reorder pages
    ...
    const finalFile = await prisma.file.create({
      data: {
        ...
        originalName: `reordered-${sourceFile.originalName}`,
